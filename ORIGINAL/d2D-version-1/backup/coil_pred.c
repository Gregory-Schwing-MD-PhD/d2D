/* 
 * compile with
 *  gcc  coil_pred.c -lm -O9 -Wall -Wextra -ansi -pedantic -o coil.x
 */
...
int do_test(char *inputfile)
{
  ...
  double mean[21], mean2[21], cmean[21];
  ...
  for(ashift=0;ashift<NSHIFT;ashift++) {
    switch(ashift)
    {
      case CA: strcpy(n1,"/coil-db/CA-uno.dat"); strcpy(n2,"/coil-db/CA-due.dat"); strcpy(n3,"/coil-db/CA-tre.dat"); 
               strcpy(n4,"/coil-db/CA-quattro.dat"); strcpy(n5,"/coil-db/CA-parm.dat"); break;
      case CB: strcpy(n1,"/coil-db/CB-uno.dat"); strcpy(n2,"/coil-db/CB-due.dat"); strcpy(n3,"/coil-db/CB-tre.dat"); 
               strcpy(n4,"/coil-db/CB-quattro.dat"); strcpy(n5,"/coil-db/CB-parm.dat"); break;
      case CO: strcpy(n1,"/coil-db/CO-uno.dat"); strcpy(n2,"/coil-db/CO-due.dat"); strcpy(n3,"/coil-db/CO-tre.dat"); 
               strcpy(n4,"/coil-db/CO-quattro.dat"); strcpy(n5,"/coil-db/CO-parm.dat"); break;
      case HN: strcpy(n1,"/coil-db/HN-uno.dat"); strcpy(n2,"/coil-db/HN-due.dat"); strcpy(n3,"/coil-db/HN-tre.dat"); 
               strcpy(n4,"/coil-db/HN-quattro.dat"); strcpy(n5,"/coil-db/HN-parm.dat"); break;
      case HA: strcpy(n1,"/coil-db/HA-uno.dat"); strcpy(n2,"/coil-db/HA-due.dat"); strcpy(n3,"/coil-db/HA-tre.dat"); 
               strcpy(n4,"/coil-db/HA-quattro.dat"); strcpy(n5,"/coil-db/HA-parm.dat"); break;
      case NH: strcpy(n1,"/coil-db/NH-uno.dat"); strcpy(n2,"/coil-db/NH-due.dat"); strcpy(n3,"/coil-db/NH-tre.dat"); 
               strcpy(n4,"/coil-db/NH-quattro.dat"); strcpy(n5,"/coil-db/NH-parm.dat"); break;
    }
    ...
    chi = sqrt(chi/count);
    fprintf(stderr,"Err %lf Cor %lf\n", chi, cor);

    pred = fopen(n6, "w"); 
    for(i=0;i<sql;i++) 
    {
      if(ecs[i]<999) 
        fprintf(pred, "%i \t %c \t %10.3lf %10.3lf \n", i+1, seq[i], tcs[i], ecs[i]);
    }
    fclose(pred);
