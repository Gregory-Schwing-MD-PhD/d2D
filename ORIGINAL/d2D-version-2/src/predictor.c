...
int do_alpha(int ashift, double *tcs, int *nseq, int sql, int ph, char *dbpath)
{
  ...
  for(i=0;i<sql;i++)
  {
    ...
    if(i>3&&nseq[i-1]!=Z&&nseq[i-2]!=Z&&nseq[i-3]!=Z&&nseq[i-4]!=Z)       tcs[i] += ad2*d2[nseq[i-4]][nseq[i]]; 
...
double do_predict(int ashift, char *bmrb, int ph, char *dbpath, char *out, int firstres, char *argv[], int argc)
{
    ...
    for(i=0;i<sql;i++) 
    {
      ...
      if(ecs[ashift][i]<900.&&ecs[ashift][i]>0.) {
        if(((ecs[ashift][i]>=alphacs[i]-wc[5][5]*sigma[0])||
         ...
         /*if((Aprob[i][ashift]>900)&&(Bprob[i][ashift]>900)&&(Cprob[i][ashift]>900)) star[i]++;*/
#ifdef LORENTZ
  ...
  for(i=0;i<sql;i++) 
  {
    /* if there is a Z star is automatically 0 so it works to split multiple chains */
    coeff=0;
    for(ashift=start;ashift<(end+1);ashift=ashift+by)
    {
...
int main(int argc, char *argv[])
{
  ...
  dbpath = getenv ("CAMDBV2");
  if (dbpath==NULL) {fprintf (stderr, "The CAMDBV2 is not set!\n Read the README file for more details!\n"); return 1;}
  ...
  if(debug) {fprintf(stdout, "In main: CAMDBV2 is %s\n", dbpath); fflush(stdout);}
